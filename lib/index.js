...
import { normalizeConfig, configSummary } from './config.js';
import { createMcpBridge, DEFAULT_DRE_FILTER, applySynapseGate } from './mcp-bridge.js';
export const name = 'dre';
/** 必需服务：tools。 */
export const inject = ['tools'];
/** 计算生效白名单：config.toolFilter（非空时完全替换默认）→ synapseEnabled 门控。 */
function resolveToolFilter(config) {
...
/** 构造始终可用的插件诊断工具。 */
function makeStatusTool(getState) {
    return {
...
export function apply(ctx, rawConfig) {
    ...
    if (config.mcpEnabled) {
        ...
        if (config.mcpFailOnStartupError) {
            ...
            return connectPromise.then(() => registerStatusAndEffect(ctx, config, () => bridge, toolFilter), (err) => {
...
/** 注册诊断工具 + 生命周期清理。 */
function registerStatusAndEffect(ctx, config, getBridge, toolFilter) {
    ctx.tools.register(makeStatusTool(() => ({
