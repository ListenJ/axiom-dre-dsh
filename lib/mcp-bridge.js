...
export function matchTool(name, filter) {
    return filter.some((f) => (f.endsWith('_') ? name.startsWith(f) : name === f));
}
/** 应用 synapseEnabled：false 时剔除突触工具。 */
export function applySynapseGate(filter, synapseEnabled) {
    if (synapseEnabled)
        return filter;
    return filter.filter((f) => f !== 'mind_synapse_' && f !== 'mind_suggest');
}
/** 从 MCP content 块提取纯文本。 */
export function extractText(content, rawName) {
    if (!Array.isArray(content))
        return `[dre:${rawName}] no text content`;
    const parts = [];
...
export function toToolDefinition(tool, opts, getClient) {
    ...
    return {
        ...
        execute: async (args, exec) => {
            ...
            if (!client)
                throw new Error(`[axiom-dre-dsh] MCP client not connected for tool ${rawName}`);
            const result = (await client.request({ method: 'tools/call', params: { name: rawName, arguments: args } }, CallToolResultSchema, { signal: exec?.signal, timeout: opts.toolCallTimeoutMs }));
            ...
            const value = { content: result.content ?? [] };
...
export function createMcpBridge(opts) {
    const state = { client: null, transport: null, disposers: [], count: 0 };
    ...
    async function connect(ctx) {
        ...
        ctx.logger?.info?.(`[axiom-dre-dsh] bridged ${selected.length}/${tools.length} DRE tools from Axiom MCP server (${opts.serverName})`);
    }
    function dispose() {
